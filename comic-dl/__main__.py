...
def download(result: dict) -> None:
    ...
    # 3. Both single and multiple downloads available on a page, prioritize segmented downloads
    ...
    # If no segmented downloads are available then default to single download link
    if not urls:
...
def select_item(items: int) -> int:
    ...
    while True:
        ...
        try:
            choice = int(choice)
            if (int(choice) < 1 or int(choice) > items): pass
            else: break
        except ValueError:
...
if __name__=="__main__":
    ...
    while i < len(sys.argv):
        ...
        if (flag == '-s' or flag == '--search'):
            ...
            search_term = " ".join(str(item) for item in sys.argv[i+1:])
